...
<section class="py-16 px-6 max-w-4xl mx-auto">
    ...
    <form action="{{ route('brands.update', $brand) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="space-y-2">
            ...
            <input type="text" name="name" value="{{ old('name', $brand->name) }}" required class="w-full px-4 py-2 border rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
        </div>
        ...
        @if ($brand->image)
            <div class="mt-4">
                <img src="{{ asset('storage/' . $brand->image) }}" alt="Brand Image" width="100" class="rounded-lg shadow-md">
            </div>
        @endif
