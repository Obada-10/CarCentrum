...
<section class="py-16 px-6 max-w-xl mx-auto">
    <h1 class="text-3xl font-bold text-secondary-800 mb-8">Categorie Bewerken</h1>
    ...
    <form action="{{ route('categories.update', $category) }}" method="POST"
        ...
        <div>
            <label for="description" class="block text-sm font-medium text-secondary-700 mb-1">Omschrijving</label>
            <textarea name="description" id="description" rows="4"
                      class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500">{{ old('description', $category->description) }}</textarea>
        </div>
